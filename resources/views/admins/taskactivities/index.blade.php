          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                ...
                <a href="/taskactivities/createdaily" class="btn btn-primary mb-3"><i class="bi bi-plus"></i> Add new daily </a>
                <a href="/taskactivities/createweekly" class="btn btn-primary mb-3"><i class="bi bi-plus"></i> Add new weekly </a>
                ...
                <table class="table table-striped datatable">
                  ...
                  <tbody>
                    ...
                    @foreach ($taskactivities as $taskactivity)
                        @if ($taskactivity->task_act > 0)
                        <tr>
                            <th scope="row">{{ $loop->iteration }}</th>
                            @if($taskactivity->report_date == '0000-00-00')
                            <td>Weekly</td>
                            @else
                            <td>Daily</td>
                            @endif
                            <td >
                                {{ ( $taskactivity->task->task != Null) ? $taskactivity->task->task : ''  }}
                            </td>
                            <td>{!! $taskactivity->task_description !!}</td>
                            <td>{{ $taskactivity->task_working_hour }}</td>
                            <td>{{ $taskactivity->task_progress }}</td>
                            <td class="text-nowrap">
                                <a href="/taskactivities/{{ $taskactivity->id }}" class="btn btn-info btn-sm" title="View">
                                    <i class="bi bi-eye"></i>
                                </a>
                                @if($taskactivity->report_date == '0000-00-00')
                                    <a href="/taskactivities/editweekly/{{ $taskactivity->id }}" class="btn btn-warning btn-sm" title="Update">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                @else
                                    <a href="/taskactivities/editdaily/{{ $taskactivity->id }}" class="btn btn-warning btn-sm" title="Update">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                @endif

                                {{-- delete post --}}
                                <form method="post" action="/taskactivities/{{ $taskactivity->id }}" class="d-inline">
                                    @method('delete')
                                    @csrf
